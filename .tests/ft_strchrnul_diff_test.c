...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_strchrnul_diff_test.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joesanto <joesanto@student.42porto.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/29 07:54:21 by joesanto          #+#    #+#             */
/*   Updated: 2025/10/02 13:05:12 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *strchrnul_diff(const char *s, int c)
{
    ...
    while (*s)
    {
        ...
        // Found a character that's different
        return ((char *)s);
    }
    
    // Check if we're looking for '\0' and reached it
    if ((char)c == '\0')
        return ((char *)s);
    
    // Reached end without finding different character, return pointer to '\0'
    return ((char *)s);
}
...
void	test(char	*tab[], size_t size, t_range range, int flags)
{
	...
	while (size--)
	{
		...
		{
			expected = strchrnul_diff(tab[size], find);
			output = ft_strchrnul_diff(tab[size], find);
			color = expected == output ? GREEN : RED;

			printf("%s", color);
			printf("Input:   \t(%p) %s\t%d\n", tab[size], tab[size], find);
			printf("Expected:\t(%p) %s\n", expected, expected);
			printf("Output:  \t(%p) %s\n", output, output);
			printf("%s", RESET);
