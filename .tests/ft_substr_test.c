...
/*   Updated: 2025/10/04 17:05:54 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test(char *tab[], int size)
{
	...
	while (size--)
	{
		str_len = ft_strlen(tab[size]) + 1;
		n = -1;
		while (++n <= str_len)
		{
			len = rand() % str_len;
			if (tab[size])
				expected_dst = substr(tab[size], n, len);
			else
				expected_dst = 0;
			output_dst = ft_substr(tab[size], n, len);
			color = !ft_strcmp(expected_dst, output_dst) ? GREEN : RED;
			...
			ATF_CHECK(!ft_strcmp(expected_dst, output_dst));
			printf("----------\n");
			free(output_dst);
			free(expected_dst);
		}
