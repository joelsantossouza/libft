...
/*   Updated: 2025/10/01 15:57:42 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test(t_input tab[], int size)
{
	...
	while (size--)
	{
		...
		bzero(expected_dst, tab[size].n);
		ft_bzero(output_dst, tab[size].n);
		color = !memcmp(output_dst, expected_dst, tab[size].n) ? GREEN : RED;

		printf("%s", color);
		printf("Input:  \t%lu\n", tab[size].n);
		ft_put_row("Expected:", (char *) expected_dst, tab[size].n);
		ft_put_row("Output:  ", (char *) output_dst, tab[size].n);
		printf("%s", RESET_COLOR);

		ATF_CHECK(!memcmp(output_dst, expected_dst, tab[size].n));
		printf("----------\n");
		free(output_dst);
		free(expected_dst);
	}
...
void	test_expect(t_input tab[], int size, int flags)
{
	...
	while (size--)
	{
		...
		if (output_dst)
			color = !memcmp(expected_dst, output_dst, tab[size].n) ? GREEN : RED;
		else
			color = expected_dst == output_dst ? GREEN : RED;

		printf("%s", color);
		printf("Input:    \t%lu\n", tab[size].n);
		printf("Expected: \t%.*s\n", (int) tab[size].n, (char*) expected_dst);
		printf("Output:   \t%.*s\n", (int) tab[size].n, (char*) output_dst);
		printf("%s", RESET_COLOR);

		if (output_dst)
			ATF_CHECK(!memcmp(expected_dst, output_dst, tab[size].n));
		else
			ATF_CHECK_EQ(expected_dst, output_dst);
		printf("----------\n");
		free(output_dst);
		free(expected_dst);
	}
