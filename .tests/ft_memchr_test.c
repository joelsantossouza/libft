...
/*   Updated: 2025/09/29 14:31:39 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test(char	*tab[], size_t size, t_range range, int flags)
{
	...
	while (size--)
	{
		...
		{
			limit = -1;
			len = ft_strlen(tab[size]) + 10;
			while (++limit < len)
			{
				expected = flags & TEST_NULLS ? 0 : memchr(tab[size], find, limit);
				output = ft_memchr(tab[size], find, limit);
