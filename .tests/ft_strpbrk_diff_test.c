...
char *strpbrk_diff(const char *s, const char *reject)
{
    const char *r;
    ...
    // If reject is empty, return first character of s (if exists)
    if (*reject == '\0')
    ...
    // All characters were in reject set
    return (NULL);
}
...
void	test(t_input tab[], size_t size)
{
	...
	while (size--)
	{
		expected = strpbrk_diff(tab[size].big, tab[size].little);
