...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_static_ultoa_base_test.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joesanto <joesanto@student.42porto.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/28 19:25:36 by joesanto          #+#    #+#             */
/*   Updated: 2025/10/13 15:29:43 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test(t_range range, char *tab[], size_t size)
{
	...
	while (size--)
	{
		...
		while (++input <= range.max)
		{
			...
			color = expected == ft_atoul_base(output, 0, tab[size]) ? GREEN : RED;

			printf("%s", color);
			printf("Input:   \t%ld - %s\n", input, tab[size]);
			printf("Expected:\t%lu (In decimal)\n", expected);
			printf("Output:  \t%s\n", output);
			printf("%s", RESET);

			ATF_CHECK(expected == ft_atoul_base(output, 0, tab[size]));
			printf("----------\n");
		}
	}
}
